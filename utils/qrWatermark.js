...
function chooseVersion(textLength, errorLevel) {
  for (let version = 1; version < QR_CAPACITIES.length; version++) {
    ...
    if (!info) continue;
    const dataCapacity = info.total - info.ec *
      (info.group1.blocks + (info.group2?.blocks ?? 0));
...
function buildData(version, errorLevel, bytes) {
  const info = QR_CAPACITIES[version][errorLevel];
  const totalDataBytes = info.group1.blocks * info.group1.data +
...
function reserveFormatAndVersion(matrix, version) {
  ...
  if (version >= 7) {
    // version information area (not fully implemented for simplicity)
  }
}
...
const FORMAT_INFO = {
  L: [0b111011111000100, 0b001011010001001, 0b101001011001000, 0b011001110000101, 0b110011000101111, 0b000011101100010, 0b100001100100011, 0b010001001101110],
  M: [0b111001011110011, 0b001001110111110, 0b101011111111111, 0b011011010110010, 0b110001100011000, 0b000001001010101, 0b100011000010100, 0b010011101011001],
  Q: [0b111110110101010, 0b001110011100111, 0b101100010100110, 0b011100111101011, 0b110110001000001, 0b000110100001100, 0b100100101001101, 0b010100000000000],
  H: [0b111100001011111, 0b001100100010010, 0b101110101010011, 0b011110000011110, 0b110100110110100, 0b000100011111001, 0b100110010111000, 0b010110111110101],
};
